...
<template>
    <div class="flex gap-2 flex-wrap">
        <select v-if="!hideCategoryFilter" :value="modelSelectedCategory" @change="onCategoryChange" class="select-modern max-w-[150px]" :aria-label="$t('dashboard.all_categories') || 'Filter by category'">
            <option value="">{{ $t('dashboard.all_categories') || 'All Categories' }}</option>
            <option v-for="cat in categories" :key="cat.id" :value="cat.id">
                {{ getTranslated(cat.name) }}
            </option>
        </select>
        <select :value="modelSortBy" @change="onSortByChange" class="select-modern" :aria-label="$t('sort.date') || 'Sort by'">
        ...
        <select :value="modelSortOrder" @change="onSortOrderChange" class="select-modern" :aria-label="$t('sort.desc') || 'Sort order'">
